...
import { ADD_EMPLOYEE_TO_ENTERPRISE, REMOVE_EMPLOYEE_FROM_ENTERPRISE, UPDATE_EMPLOYEE } from "@graphql/mutations";
...
function EmployeeCard({ title, user, token, refetch }:
  { title: string, id: string, user: any, token: string, refetch: (o: any) => void }) {
  ...
  return (
    <div style={{ backgroundColor: "gray", padding: "30px", borderRadius: "10px" }}>
      <section style={{ display: "flex", justifyContent: "space-between" }}>
        <h1>{title}</h1>
        <button style={{ color: "white", backgroundColor: "black", padding: "10px" }} onClick={() => setOpen(!open)}>{!open ? "Open" : "Close"}</button>
      </section>
      <label style={{ backgroundColor: "white", padding: "3px" }}>{user.id}</label>
              ...
              return (
                <form style={{ marginTop: "4px" }} onSubmit={handleSubmit}>
                  ...
                  <div style={{ display: "flex" }}>
                    <FormikControl control="media-image" name={`self_portrait_image`} />
                    <label style={{ color: "white", marginTop: "3pc" }}>Portrait Image</label>
                  </div>
                  <div style={{ display: "flex" }}>
                    <FormikControl control="media-image" name={`self_landscape_image`} />
                    <label style={{ color: "white", marginTop: "3pc" }}>Landscape Image</label>
                  </div>
                  <div style={{ display: "flex" }}>
                    <FormikControl control="media-image" name={`profile_picture`} />
                    <label style={{ color: "white", marginTop: "3pc" }}>Profile Image</label>
                  </div>
                </form>
              )
            }
...
const Enterprise = (props: Props) => {
  ...
  const { data, error, refetch } = useQuery(GET_ENTERPRISE_EMPLOYEES, { variables: { token: cookies.userAuthToken } });
  ...
  return (
    <div>
      <section>
        <input style={{ padding: "10px" }} placeholder="New Employee Email" onChange={(e) => setNewEmployeeEmail(e.target.value)} />
        <FActionsBtn
