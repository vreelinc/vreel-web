export interface LinksDataTypes {
    id:number,
    image:string,
    text:string
}

// export const LinksData = [
//     {data1:{
//         id:1,
//         image:"/assets/images/vLinks1.svg",
//         text:"Dow Inks PCR Supply Deal With Avangard Innovative",
//     },
//     data2:{
//         id:2,
//         image:"/assets/images/honey.svg",
//         text:"Honeywell Partners with Avangard Innovative"
//     },
//     data3:{
//         id:3,
//         image:"/assets/images/vLinks2.svg",
//         text:"Honeywell Partners with Avangard Innovative"
//     },
//     data4:{
//         id:4,
//         image:"/assets/images/man.svg",
//         text:"Working to scale post-consumer resin recycling"
//     },
//     data5:{
//         id:5,
//         image:"/assets/images/man.svg",
//         text:" Avangard Innovative: Named a 2020 Top Foam Technology Solution Provider"
//     },
//     data6:{
//         id:6,
//         image:"/assets/images/sceneray.svg",
//         text:" How a partnership created a ring carrier recycling program"
//     },
// },
// {data1:{
//         id:1,
//         image:"/assets/images/vLinks1.svg",
//         text:"Dow Inks PCR Supply Deal With Avangard Innovative",
//     },
//     data2:{
//         id:2,
//         image:"/assets/images/vLinks2.svg",
//         text:"Honeywell Partners with Avangard Innovative"
//     },
//     data3:{
//         id:3,
//         image:"/assets/images/vLinks3.svg",
//         text:"Honeywell Partners with Avangard Innovative"
//     },
//     data4:{
//         id:4,
//         image:"/assets/images/Events1.svg",
//         text:"Working to scale post-consumer resin recycling"
//     },
//     data5:{
//         id:5,
//         image:"/assets/images/Events2.svg",
//         text:" Avangard Innovative: Named a 2020 Top Foam Technology Solution Provider"
//     },
//     data6:{
//         id:6,
//         image:"/assets/images/Events3.svg",
//         text:" How a partnership created a ring carrier recycling program"
//     },
// }
    
// ]

 const Data:LinksDataTypes[] = [
    {
        id: 1,
        image: "/assets/images/vLinks1.svg",
        text: "Dow Inks PCR Supply Deal With Avangard Innovative",
    },
    {
        id: 2,
        image: "/assets/images/vLinks2.svg",
        text:"Working to scale post-consumer resin recycling"
    },
    {
        id: 3,
        image: "/assets/images/honey.svg",
        text:"Honeywell Partners with Avangard Innovative"
    },
    {
        id: 4,
        image: "/assets/images/man.svg",
        text:" Avangard Innovative: Named a 2020 Top Foam Technology Solution Provider"
    },
    {
        id: 5,
        image: "/assets/images/vLinks3.svg",
        text:" How a partnership created a ring carrier recycling program"
    },
    {
        id: 6,
        image: "/assets/images/sceneray.svg",
        text: "Avangard Innovative Partners with Hi-Cone Worldwide",
    },
    {
        id: 7,
        image: "/assets/images/Events1.svg",
        text:"Working to scale post-consumer resin recycling"
    },
    {
        id: 8,
        image: "/assets/images/Events2.svg",
        text:"Honeywell Partners with Avangard Innovative"
    },
    {
        id: 9,
        image: "/assets/images/Events3.svg",
        text:" Avangard Innovative: Named a 2020 Top Foam Technology Solution Provider"
    },
    {
        id: 10,
        image: "/assets/images/female.png",
        text:" How a partnership created a ring carrier recycling program"
    },
    {
        id: 11,
        image: "/assets/images/man.svg",
        text: "Avangard Innovative Partners with Hi-Cone Worldwide",
    },
    {
        id: 12,
        image: "/assets/images/sceneray.svg",
        text: "Dow Inks PCR Supply Deal With Avangard Innovative",
    },
]

export const LinksData:Array<LinksDataTypes>[] = []
const  size = 6;
Data.forEach((item)=>{
    if(!LinksData.length || LinksData[LinksData.length - 1].length === size){
        LinksData.push([]);
    }
    LinksData[LinksData.length -1 ].push(item)
});

